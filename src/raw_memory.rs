...
extern "C" {
    ...
    #[wasm_bindgen(structural, static_method_of = RawMemory, getter = segments)]
...
impl RawMemory {
    ...
    pub fn read_segment(segment_id: u8) -> Option<String> {
        RawMemory::segments().get(segment_id)
    }

    pub fn write_segment(segment_id: u8, content: &str) {
        JsHashMap::<u8, JsString>::from(RawMemory::segments_internal())
            .set(segment_id, JsString::from(content))
    }
